  ...
  return (
    <div className="px-5 py-6 sm:px-7">
      <PageHeader
        ...
        subtitle="Release ledger for define.xml publications, CRF specifications and MDR snapshots — plus the per-variable version census for the master repository."
      >
        <button
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_320px]">
        ...
        <div>
          <div className="flex flex-col gap-2.5">
            {releases.map((r) => {
              ...
              return (
                <div key={r.id} className={`rounded-lg border bg-panel shadow-xs transition-colors ${isExp ? "border-sdtm/50" : "border-line"}`}>
                  <button onClick={() => setExpanded(isExp ? null : r.id)} className="flex w-full flex-wrap items-center gap-x-5 gap-y-1.5 px-5 py-4 text-left hover:bg-raise/60 cursor-pointer">
                    <IconChevron size={12} className={`text-faint transition-transform duration-200 ${isExp ? "rotate-90" : ""}`} />
                    <span className="w-[170px]">
                      <span className="block font-display text-[15px] font-bold text-ink">{r.type} <span className="text-sdtm">{r.version}</span></span>
                    </span>
                    <StatusBadge status={r.status} small />
                    <span className="flex gap-2 font-mono text-[10px] tabular font-semibold">
                      <span className="text-sdtm">+{r.added}</span>
                      <span className="text-crf">~{r.changed}</span>
                      <span className="text-adam">−{r.deprecated}</span>
                    </span>
                  ...
                  {isExp && (
                    <div className="anim-fade grid gap-x-8 gap-y-2 border-t border-line bg-raise/30 px-5 py-4 text-[11.5px] sm:grid-cols-2">
        ...
        <div className="flex flex-col gap-4">
          <section className="rounded-lg border border-line bg-panel p-4 shadow-sm">
            <h3 className="font-display text-[13.5px] font-bold">Version census — master</h3>
            ...
            <ul className="mt-3 flex flex-col gap-2">
              {versionCensus.map((r) => (
                <li key={r.version} className="flex items-center gap-2.5">
                  <span className="w-10 shrink-0 font-mono text-[10px] font-semibold text-[#7a4f9b]">v{r.version}</span>
                  <div className="h-[9px] flex-1 overflow-hidden rounded-full bg-raise">
                    <div className="h-full rounded-full bg-[#7a4f9b]/65 transition-all duration-700" style={{ width: `${(Number(r.n) / maxN) * 100}%` }} />
                  </div>
          ...
          <section className="rounded-lg border border-line bg-panel p-4 shadow-sm">
            ...
            <ul className="mt-3 flex flex-col">
              {ledger.map((l, i) => (
                <li key={l.id} className="relative border-l border-line pb-3 pl-3.5 last:pb-0">
                  <span className="absolute -left-[4px] top-1 h-2 w-2 rounded-full" style={{ background: i === 0 ? "#7a4f9b" : "#a0b2be" }} />
                  <p className="font-mono text-[10px]">
                    <span className="font-semibold text-[#7a4f9b]">v{l.version}</span>
                    <span className="ml-1.5 text-ink font-medium">{l.domain}.{l.name}</span>

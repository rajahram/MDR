...
import type { AuditRow, CodelistRow, ReleaseRow, Status, StudyRow, VariableRow } from "../data/types";
...
const S_COLOR: Record<string, string> = { DRAFT: "#93a8ad", "IN REVIEW": "#f2ac3c", ACTIVE: "#38c7a6", DEPRECATED: "#f27059" };
...
export default function Dashboard() {
  const { db, v, setView, study, setStudy } = useStore();

  const data = useMemo(() => {
    if (!db) return null;
    const studies = all<StudyRow>(db, "SELECT * FROM studies ORDER BY study_id");
    ...
    return { studies, vars, domains, codelists, ctTerms, vlmN, crfPages, crfFields, tflN, releases, audit, gapsCrf, gapsSdtm, varCounts, allCounts, ctUsage, auditTotal };
  }, [db, v]);
  ...
  return (
    <div ref={ref} className="reveal px-5 py-6 sm:px-7">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2 rounded-lg border border-line bg-deep/70 px-4 py-3">
          <IconDb size={16} className="text-sdtm" />
          <div className="leading-tight">
            <p className="font-mono text-[11px] font-semibold text-ink">master-mdr.sqlite</p>
            <p className="font-mono text-[9.5px] text-faint">{data.auditTotal} audit events · persisted locally</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4 xl:grid-cols-8">
        <StatCard label="Studies" value={data.studies.length} sub="in master scope" color="#c9b3ff" />
        <StatCard label="Domains" value={data.domains.length} sub="SDTM + ADaM" />
        <StatCard label="Variables" value={varsN} sub={`${activeVars} active`} />
        <StatCard label="CT terms" value={data.ctTerms} sub={`${data.codelists.length} codelists`} color="#f2ac3c" />
        <StatCard label="VLM records" value={data.vlmN} sub="value-level rules" color="#7fb7e6" />
        <StatCard label="CRF forms" value={data.crfPages} sub={`${data.crfFields} fields`} color="#f27059" />
        <StatCard label="TFL refs" value={data.tflN} sub="protocol shells" />
        <StatCard label="Audit events" value={auditN} sub="append-only" color="#93a8ad" />
      </div>

      {/* status distribution */}
      <section className="mt-6 rounded-lg border border-line bg-deep/70 p-5">
        ...
        <p className="mt-2.5 text-[11px] text-faint">
          Draft metadata enters as <span className="text-[#93a8ad]">DRAFT</span>, passes standards review <span className="text-[#f2ac3c]">IN REVIEW</span>, is activated with an electronic signature <span className="text-sdtm">ACTIVE</span>, and retires with a documented reason <span className="text-crf">DEPRECATED</span>.
        </p>
      </section>
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-3">
        ...
        <section className="xl:col-span-2">
          <h2 className="mb-3 flex items-center gap-2 font-display text-[15px] font-bold">
            <IconDb size={15} className="text-sdtm" /> Study dossiers
          </h2>
          <div className="grid gap-4 md:grid-cols-2">
            {data.studies.map((s) => {
              const sVars = data.vars.filter((x) => x.study_id === s.study_id);
              const sDoms = data.domains.filter((x) => x.study_id === s.study_id);
              const active = sVars.filter((x) => x.status === "ACTIVE").length;
              const counts: Record<string, number> = {};
              for (const st of STATUSES) counts[st] = sVars.filter((x) => x.status === st).length;
              return (
                <button
                  key={s.study_id}
                  onClick={() => { setStudy(s.study_id); setView("explorer"); }}
                  className={`group rounded-lg border bg-deep/70 p-4 text-left transition-all hover:-translate-y-0.5 hover:shadow-xl hover:shadow-black/30 ${study === s.study_id ? "border-sdtm/50" : "border-line hover:border-line/80"}`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="font-display text-[17px] font-bold tracking-tight">{s.study_id}</p>
                      <p className="mt-0.5 text-[11.5px] text-dim">{s.study_name}</p>
                    </div>
                    <StatusBadge status={s.status} small />
                  </div>
                  <div className="mt-3 grid grid-cols-2 gap-x-3 gap-y-1.5 font-mono text-[10.5px]">
                    <span className="text-faint">PROTOCOL</span><span className="text-ink">{s.protocol}</span>
                    <span className="text-faint">PHASE</span><span className="text-ink">{s.phase} · {s.ta}</span>
                    <span className="text-faint">CRF</span><span className="truncate text-crf" title={s.crf_name}>{s.crf_name} {s.crf_version}</span>
                    <span className="text-faint">IG</span><span className="text-ink">{s.sdtm_ig} / {s.adam_ig}</span>
                  </div>
                  <div className="mt-3.5">
                    <StackedBar counts={counts} height={7} />
                    <p className="mt-1.5 font-mono text-[9.5px] text-faint">
                      {sDoms.length} domains · {sVars.length} variables · {active} active
                    </p>
                  </div>
                </button>
              );
          ...
          {/* domain coverage */}
          <div className="mt-4 rounded-lg border border-line bg-deep/70 p-4">
            <div className="flex items-center justify-between">
              <h3 className="font-display text-[13.5px] font-bold">Domain census by standard</h3>
              <button onClick={() => setView("domains")} className="font-mono text-[10px] text-sdtm underline-offset-2 hover:underline">open registry →</button>
            </div>
            <div className="mt-3 grid grid-cols-2 gap-4">
              {(["SDTM", "ADaM"] as const).map((std) => {
                const doms = data.domains.filter((d) => d.standard === std);
                const nvars = data.vars.filter((x) => x.standard === std).length;
                return (
                  <div key={std}>
                    <div className="flex items-baseline justify-between">
                      <span className="font-mono text-[11px] font-semibold" style={{ color: std === "SDTM" ? "#38c7a6" : "#f2ac3c" }}>{std}</span>
                      <span className="font-mono text-[9.5px] text-faint">{doms.length} domains · {nvars} vars</span>
                    </div>
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {doms.map((d) => (
                        <span key={`${std}${d.id}`} className="rounded-sm border border-line/70 px-1.5 py-0.5 font-mono text-[9.5px] text-dim" title={d.code}>
                          <span style={{ color: S_COLOR[d.status] ?? "#93a8ad" }}>●</span> {d.code}
                        </span>
                      ))}
                    </div>
                  </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <section className="rounded-lg border border-line bg-deep/70 p-4">
            ...
            <ul className="mt-3 flex flex-col">
              {data.releases.slice(0, 5).map((r) => (
                <li key={r.id} className="group relative border-l border-line/70 pb-3.5 pl-3.5 last:pb-0">
                  <span className="absolute -left-[4.5px] top-1 h-2 w-2 rounded-full" style={{ background: r.status === "RELEASED" ? "#38c7a6" : "#f2ac3c" }} />
                  <p className="font-mono text-[10.5px]">
                    <span className="font-semibold text-ink">{r.study_id} · {r.type} {r.version}</span>
                    <span className={`ml-2 rounded-sm px-1 py-px text-[8.5px] ${r.status === "RELEASED" ? "bg-sdtm/15 text-sdtm" : "bg-[#f2ac3c1f] text-[#f2ac3c]"}`}>{r.status}</span>
                  </p>
          ...
          <section className="rounded-lg border border-line bg-deep/70 p-4">
            <div className="flex items-center justify-between">
              <h3 className="flex items-center gap-2 font-display text-[13.5px] font-bold"><IconTag size={14} className="text-[#f2ac3c]" /> Terminology hotspots</h3>
            ...
            <ul className="mt-2.5 flex flex-col gap-1.5">
              {data.ctUsage.map((c) => (
                <li key={c.code} className="flex items-center gap-2.5">
                  <span className="w-16 shrink-0 font-mono text-[10px] font-semibold text-[#f2ac3c]">{c.code}</span>
                  <div className="h-[7px] flex-1 overflow-hidden rounded-full bg-raise/70">
                    <div className="h-full rounded-full bg-[#f2ac3c]/70 transition-all duration-700" style={{ width: `${Math.min(100, (c.used / (data.ctUsage[0]?.used || 1)) * 100)}%` }} />
                  </div>
          ...
          <section className="rounded-lg border border-line bg-deep/70 p-4">
            <div className="flex items-center justify-between">
              <h3 className="flex items-center gap-2 font-display text-[13.5px] font-bold"><IconClock size={14} className="text-[#93a8ad]" /> Audit stream</h3>
              ...
              {data.audit.slice(0, 6).map((a) => (
                <li key={a.id} className="rounded-md border border-line/50 bg-panel/40 px-2.5 py-2">
                  <p className="flex items-center gap-2 font-mono text-[9.5px]">
                    ...
                    <span className="truncate text-dim">{a.record}</span>
      ...
      {/* gap strip */}
      <section className="mt-6 flex flex-wrap items-center gap-x-8 gap-y-2 rounded-lg border border-crf/25 bg-crf/6 px-5 py-3.5">
